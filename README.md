...
```sh
./target/release/node-nft_currency -h
```
...
```bash
./target/release/node-nft_currency --dev
```

Purge the development chain's state:

```bash
./target/release/node-nft_currency purge-chain --dev
```

Start the development chain with detailed logging:

```bash
RUST_BACKTRACE=1 ./target/release/node-nft_currency -ldebug --dev
```
...
$ ./target/release/node-nft_currency --dev --base-path ./my-chain-state/
...
```shell
./target/release/node-nft_currency --help
```
...
template pallet that is [defined in the `pallets`](pallets/nft_currency/src/lib.rs) directory.
